        ...
        try {
            ...
            LocalBroadcastManager.getInstance(this).registerReceiver(new BroadcastReceiver() {
                ...
                public void onReceive(Context context, Intent intent) {
                    finish();
                } // Stop activity if bluetooth is disconnected
...
//      recordingsDB.execSQL("DROP TABLE IF EXISTS Recording;");
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            case R.id.action_record:
                ...
                setRecordingIcon();
                if (!isRecording)
                    saveRecording(); // If the user stopped the recording then save in DB
                return true;
    ...
    private void initWidgets() {
        ...
        blWeightTw = findViewById(R.id.bl_weight);
    }
    ...
    private void setWarningNotificationWeight() {
        NotificationCompat.Builder mBuilder = new NotificationCompat.Builder(this, CHANNEL_ID)
                .setSmallIcon(R.drawable.robocar)
                .setContentTitle(getString(R.string.controller_notification_title_weight))
                .setPriority(NotificationCompat.PRIORITY_DEFAULT)
                .setStyle(new NotificationCompat.BigTextStyle()
                        .bigText(getString(R.string.controller_notification_content_weight)))
                .setAutoCancel(true);
    ...
    private class BluetoothBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            blWeightTw.setText(getString(R.string.controller_weight, weight));


            if (weight >= 1) {
                setWarningNotificationWeight();
                blWeightTw.setText(getString(R.string.controller_weight, weight)); // Обновление для float
                blWeightTw.setTextColor(Color.RED);
            } else {
                blWeightTw.setText(getString(R.string.controller_weight, weight)); // Обновление для float
                blWeightTw.setTextColor(Color.BLACK);
            }
